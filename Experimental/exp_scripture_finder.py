...
class ScriptureFinder(object):
    ...
    def run(self, source):
        if source != []:
            thread_assign = int(len(source) / 8)
            self.source[0] = source[:thread_assign]
            self.source[1] = source[thread_assign: thread_assign * 2]
            self.source[2] = source[thread_assign * 2: thread_assign * 3]
            self.source[3] = source[thread_assign * 3: thread_assign * 4]
            self.source[4] = source[thread_assign * 4: thread_assign * 5]
            self.source[5] = source[thread_assign * 5: thread_assign * 6]
            self.source[6] = source[thread_assign * 6: thread_assign * 7]
            self.source[7] = source[thread_assign * 7:]

            self.t0.run()

        else:
            self.t0 = threading.Thread(target=self.create_thread, args=('t0', self.source[0]))
            print('t0 ready')
            self.t1 = threading.Thread(target=self.create_thread, args=('t1', self.source[1]))
            print('t1 ready')
            self.t2 = threading.Thread(target=self.create_thread, args=('t2', self.source[2]))
            print('t2 ready')
            self.t3 = threading.Thread(target=self.create_thread, args=('t3', self.source[3]))
            print('t3 ready')
            self.t4 = threading.Thread(target=self.create_thread, args=('t4', self.source[4]))
            print('t4 ready')
            self.t5 = threading.Thread(target=self.create_thread, args=('t5', self.source[5]))
            print('t5 ready')
            self.t6 = threading.Thread(target=self.create_thread, args=('t6', self.source[6]))
            print('t6 ready')
            self.t7 = threading.Thread(target=self.create_thread, args=('t7', self.source[7]))
            print('t7 ready')
        ...
        while self.t0.is_alive() or self.t1.is_alive() or self.t2.is_alive() or self.t3.is_alive() \
                or self.t4.is_alive() or self.t5.is_alive() or self.t6.is_alive() or self.t7.is_alive():
            pass
        print(self.instances['t0'].scripture_list)
        print(self.instances['t1'].scripture_list)
        print(self.instances['t2'].scripture_list)
        print(self.instances['t3'].scripture_list)
        print(self.instances['t4'].scripture_list)
        print(self.instances['t5'].scripture_list)
        print(self.instances['t6'].scripture_list)
        print(self.instances['t7'].scripture_list)

    def create_thread(self, thread, source):
        if source == []:
            self.instances[thread] = Finder()
        else:
            self.instances[thread].run(source)


class Finder(object):
    def __init__(self):
        self.conn = sqlite3.connect('MemScript_E.db')
    ...
    def split_long(self, bk, v):
        ...
        vrs_in_chap = self.cur.execute("SELECT ver FROM \'" + bk + "\' WHERE chap = ? ", (beg_chap,)).fetchall()
        ...
        for _ in range(0, chaps_inbetween):
            ...
            vrs_in_chap = self.cur.execute("SELECT ver FROM \'" + bk + "\' WHERE chap = ? ", (mid_chap,)).fetchall()
            len_of_chap = len(vrs_in_chap)
    ...
    def get_full_chap(self, bk, ch):
        ...
        if '-' in ch:
            ...
            for i in range(num_of_chaps):
                chap = int(beg_chap) + i
                vrs_in_chap = self.cur.execute("SELECT ver FROM \'" + bk + "\' WHERE chap = ? ", (chap, )).fetchall()
                for vrs in vrs_in_chap:
                    parsed.append(bk + '/' + str(chap) + '/' + str(vrs[0]))
        else:
            vrs_in_chap = self.cur.execute("SELECT ver FROM \'" + bk + "\' WHERE chap = ? ", (ch,)).fetchall()
